...
# ============================================================
# Random Forest Regression with CV + Test Hyperparameter Analysis
# ============================================================
def train_random_forest():
    ...
    # Hyperparameter grid
    # --------------------------------------------------------
    n_estimators_list = [50, 100, 150, 200, 250, 300, 350, 450, 500,]
    max_depth_list =    [5, 10, 15, 20, 25, 30, 35, 40, 45]
    ...
    ax.set_title("Random Forest: Actual vs Predicted OptiTrack Trajectory")
    ax.set_xlabel("X (mm)")
    ax.set_ylabel("Y (mm)")
    ax.set_zlabel("Z (mm)")
    ax.legend()
    ...
    # Convert None depth to numeric for plotting
    D_plot = np.where(D == None, -1, D).astype(float)
    ...
    def plot_surface(Z, title, zlabel, cmap):
        ...
        # ---- Mark CV minimum ----
        ax.scatter(
            D_plot[best_cv_idx[0], best_cv_idx[1]],
        ...
        # ---- Mark Test minimum ----
        ax.scatter(
            D_plot[best_test_idx[0], best_test_idx[1]],
        ...
        ax.set_xlabel("Max Depth (-1 = None)")
    ...
    plot_surface(
        cv_mse,
        "5-Fold CV MSE Hyperparameter Surface (Random Forest)",
        "5-Fold CV MSE",
        cm.viridis
    )
    ...
    plot_surface(
        test_mse,
        "Test MSE Hyperparameter Surface (Random Forest)",
        "Test MSE",
        cm.plasma
    )
